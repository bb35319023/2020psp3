...
l.80: リストの先頭に市町村のデータを代入する。

l.81: pNextにリストの先頭アドレスを代入する。

l.83: リストの先頭アドレスを返す。

l.107: int型の変数posに0を代入する。

l.108: int型の変数resultに-1を代入する。

l.109: 変数を用意する。

l.110: pNodeにリストの先頭アドレスを代入する。

l.112: pNodeがNULLでない間113行目から119行目を繰り返す。

l.113: IDが合致した場合114行目から116行目までを実行する。

l.114: City* pCityにIDが合致した市町村のデータを代入する。

l.115: resultにposを代入する。

l.116: 繰り返し処理を終了する。

l.118: posに1を足す。

l.119: pNodeが次の要素をさすようにする。
