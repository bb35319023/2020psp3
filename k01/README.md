...
## ソースコードの説明
l.68: int型の変数pos、resultの宣言。

l.71: pos=0から始め、pos < sizeの間以下を実行しposに1を加えることを繰り返す。

l.72: key == arrayCity[pos].idなら、resultにposを代入。

l.77: resultの値を返す。

l.83: int型の変数pos、resultの宣言。

l.86: leftに0を代入。

l.87: rightにMAX_CITY - 1を代入。

l.89: left <= rightの間、以下を繰り返す。

l.90: posにleft + (right - left) / 2を代入。

l.91: key == arrayCity[pos].idなら、resultにposを代入。

l.94: key > arrayCity[pos].idなら、leftにpos + 1を代入。

l.96: 違うなら、rightにpos - 1を代入。
