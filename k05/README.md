...
## ソースコードの説明
l.109: int型の配列visited、int型の変数i、j、val、countの宣言。

l.111: visitedを初期化する。

l.115: スタックを初期化する。

l.116: スタックに出発点をプッシュする。

l.117: 「深さ優先探索で到達した駅」を出力。

l.119: スタックが空でない間、120行目から127行目を繰り返す。

l.120: スタックから値をポップし、valに代入する。

l.121: 行ったことがない場所なら122行目から127行目を実行する。
...
l.125: その場所から行ける場所をスタックにプッシュする。

l.132: countとsizeが等しいなら「深さ優先探索ですべての駅に到達可能」を出力。

l.200: int型の配列visited、int型の変数i、j、val、countの宣言。
...
l.208: 「幅優先探索で到達した駅」を出力。

l.210: キューが空でない間、211行目から218行目を繰り返す。

l.211: キューから値を取り出し、valに代入する。

l.212: 行ったことがないところなら213行目から218行目を実行する。
...
l.216: その場所から行ける場所をキューに入れる。、

l.223: countとsizeが等しいなら「幅優先探索ですべての駅に到達可能」を出力。
## 出力結果
